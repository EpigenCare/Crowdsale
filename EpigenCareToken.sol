...
// File: contracts/EpigenCareToken.sol
...
contract EpigenCareToken is Ownable, StandardToken {
  ...
  function updateSendWhitelist(address[] addresses, bool setTo) onlyOwner {
    for(uint i = 0; i < addresses.length; i++) {
      sendWhitelist[addresses[i]] = setTo;
    }
  }

  function updateReceiveWhitelist(address[] addresses, bool setTo) onlyOwner {
    for(uint i = 0; i < addresses.length; i++) {
      receiveWhitelist[addresses[i]] = setTo;
    }
  }
}
